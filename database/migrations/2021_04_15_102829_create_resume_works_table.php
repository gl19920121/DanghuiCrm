...
class CreateResumeWorksTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('resume_works', function (Blueprint $table) {
            ...
            $table->enum('company_nature', ['foreign', 'joint_venture', 'private', 'state', 'listed', 'government', 'institution', 'other'])->nullable()->comment('性质');
            $table->integer('company_scale')->nullable()->comment('规模');
            ...
            $table->json('company_industry')->nullable()->comment('所属行业');
            $table->float('salary', 8, 1)->comment('月薪');
            $table->integer('salary_count')->comment('月薪');
            ...
            $table->text('work_desc')->comment('工作描述');
